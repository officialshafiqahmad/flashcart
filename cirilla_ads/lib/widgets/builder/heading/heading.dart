...
class HeadingWidget extends StatelessWidget with Utility, NavigationMixin, ContainerMixin {
  ...
  Widget build(BuildContext context) {
    ...
    //leading
    String title = ConvertData.textFromConfigs(headingData['title'], languageKey);
    ...
    //trailing
    String actionTitle = ConvertData.textFromConfigs(headingData['actionTitle'], languageKey);
    ...
    Color actionIconColor =
        ConvertData.fromRGBA(get(styles, ['actionIconColor', themeModeKey], {}), theme.iconTheme.color);
    ...
    switch (layout) {
      case "divider":
        return Heading(
          ...
          trailing:
              enableIconAction == true ? CirillaIconBuilder(data: iconAction, size: 16, color: actionIconColor) : null,
                  ...
                  child: Row(
                    children: [
                      ...
                      if (enableIconAction == true)
                        CirillaIconBuilder(data: iconAction, size: 16, color: actionIconColor)
                    ],
                  ),
                )
              ],
      ...
      default:
        return Heading(
          ...
          see: buildText(context, actionTitle, actionTitleStyle),
          trailing: enableIconAction == true ? CirillaIconBuilder(data: iconAction, size: 16, color: actionIconColor) : null,
          background: background,
          onTap: () => navigate(context, action),
        );
    }
    ...
    return Stack(
      children: [
        Container(
          padding: ConvertData.space(padding, 'padding'),
          margin: ConvertData.space(margin, 'margin'),
          decoration: decorationColorImage(color: background).copyWith(
        ...
        if (dividerHeight != null && dividerHeight > 0)
          Positioned(
            ...
            child: Container(
              ...
              decoration: decorationColorImage(color: secondBackground).copyWith(
                borderRadius: BorderRadius.only(
                  topLeft: Radius.circular(borderRadiusTopLeft),
                  topRight: Radius.circular(borderRadiusTopRight),
                  bottomLeft: Radius.circular(borderRadiusBottomLeft),
