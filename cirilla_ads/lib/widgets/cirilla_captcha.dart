...
class CirillaCaptchaWrap extends StatelessWidget {
  ...
  final void Function(String captcha, String phrase, dynamic data)? submitData;
  final bool isOnPressedCallBackData;
  final bool enable;
  ...
  const CirillaCaptchaWrap({
    ...
    this.submitData,
    this.enable = true,
    this.isOnPressedCallBackData = false,
  }) : super(key: key);

  Future<void> _showMyDialog(BuildContext context, dynamic data) async {
    if (!enable) {
      if (submitData != null) {
                ...
                Flexible(
                  child: _CirillaCaptchaForm(
                    submit: (captcha, phrase) {
                      ...
                      if (submitData != null) {
                        return submitData!(captcha, phrase, data);
                      }
                      submit!(captcha, phrase);
                    },
                  ),
  ...
  Widget build(BuildContext context) {
    if (isOnPressedCallBackData) {
      return buildButton(
        (data) => _showMyDialog(context, data),
      );
    } else {
      return buildButton(
