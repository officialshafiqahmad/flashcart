...
class _VendorDataScreenState extends State<VendorDataScreen>
  ...
  List<String> tabs = ['products', 'about', 'reviews'];
  ...
  late VendorReviewListStore _vendorReviewStore;
  ...
  void initState() {
    ...
    _vendorReviewStore = VendorReviewListStore(
      widget.store!.requestHelper,
      storeId: widget.vendor.id,
    );
    init();
  }
  ...
  void _onScroll() {
    ...
    if (_visitTab == 'reviews') {
      if (!_controller.hasClients || _vendorReviewStore.loading || !_vendorReviewStore.canLoadMore) return;
      final thresholdReached = _controller.position.extentAfter < 10;

      if (thresholdReached) {
        _vendorReviewStore.getStoreReviews();
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      key: _scaffoldKey,
      bottomSheet: _visitTab == 'reviews'
          ? BottomWriteReview(
              vendor: vendor,
              vendorReviewStore: _vendorReviewStore,
            )
          : null,
      drawer: Drawer(
  ...
  List<Widget> childrenTab(
    ...
    required VendorReviewListStore vendorReviewStore,
    String refinePosition = Strings.refinePositionBottom,
    String refineItemStyle = Strings.refineItemStyleListTitle,
  }) {
    switch (visitTab) {
      ...
      case 'reviews':
        return [
          CupertinoSliverRefreshControl(
            onRefresh: vendorReviewStore.refresh,
          ...
          SliverPadding(
            ...
            sliver: SliverToBoxAdapter(
              child: ReviewList(
                data: vendorReviewStore.reviews,
                loading: vendorReviewStore.loading,
                perPage: vendorReviewStore.perPage,
              ),
            ),
          ),
          if (vendorReviewStore.loading)
            SliverToBoxAdapter(
              child: Padding(
                padding: const EdgeInsets.only(bottom: 66),
                child: buildLoading(context, isLoading: vendorReviewStore.canLoadMore),
              ),
            ),
        ];
      case 'polices':
