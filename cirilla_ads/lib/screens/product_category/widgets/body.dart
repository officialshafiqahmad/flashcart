...
abstract class Body extends StatefulWidget with Utility {
  ...
  // Build Banner
  Widget? buildBanner(BuildContext context, {Map<String, dynamic>? configs, required String imageKey});
...
class _BodyState extends State<Body> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    Widget? banner = widget.buildBanner(context, configs: widget.configs, imageKey: widget.imageKey);
    ...
    Widget body = widget.buildBody(
      context,
      parent: widget.categories?.isNotEmpty == true? widget.categories![_index] : null,
      tab: tabs,
      appBar: appBar,
      banner: banner,
      widgetConfig: widget.widgetConfig,
...
class BannerWidget extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    String? imageBanner = ConvertData.imageFromConfigs(get(configs, ['imageBanner']), imageKey);
