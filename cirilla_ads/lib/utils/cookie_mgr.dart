...
const _kIsWeb =
    bool.hasEnvironment('dart.library.js_util') ? bool.fromEnvironment('dart.library.js_util') : identical(0, 0.0);
...
class CookieManager extends Interceptor {
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) {
    if (options.uri.queryParameters.containsKey('clear-cookies')) {
      cookieJar.deleteAll().then((_) => handler.next(options));
    } else {
      if (options.headers.containsKey('Authorization') || options.uri.queryParameters.containsKey('bypass-cookies')) {
        handler.next(options);
        return;
      }
      cookieJar.loadForRequest(options.uri).then((cookies) {
        final previousCookies = options.headers[HttpHeaders.cookieHeader] as String?;
        final newCookies = getCookies([
          ...?previousCookies?.split(';').where((e) => e.isNotEmpty).map((c) => Cookie.fromSetCookieValue(c)),
          ...cookies,
        ]);
  ...
  Future<void> _saveCookies(Response response) async {
    if (!response.requestOptions.queryParameters.containsKey('set-cookies')) {
      return;
    }
    ...
    final List<Cookie> cookies = setCookies
        ...
        .where((cookie) => cookie.value != 'deleted')
        .toList();
