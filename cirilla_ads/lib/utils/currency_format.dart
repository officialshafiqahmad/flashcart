...
String? convertCurrency(BuildContext context, {String? price, String? currency, int? unit}) {
  ...
  Map<String, dynamic> firstCurrencyInfo = settingStore.currencies[settingStore.currencies.keys.first];
  Map<String, dynamic> newCurrencyInfo = settingStore.currencies[settingStore.currency!] ?? firstCurrencyInfo;
...
String convertCurrencyFromData({String? price, int? unit, String? currencySymbol}){
  int u = unit != null && unit > 0 ? pow(10, unit) as int : 1;
  String currencyPrice = (ConvertData.stringToDouble(price) / u).toString();
    NumberFormat f = NumberFormat.currency(
  ...
  return f.format(ConvertData.stringToDouble(currencyPrice));
}
